...
public:
    ...
    void addOutput() {
        ...
        output->histograms[Systematic::syst_id_nominal] = std::move(make_unique<TH1D>("h", "h", 100, 0, 300));
        ...
        for (auto systematic : systematics_event) {
            output_systev->histograms[systematic] = std::move(make_unique<TH1D>("h", "h", 100, 0, 300));
        ...
        for (auto systematic : systematics_weight) {
            output_systw->histograms[systematic] = std::move(make_unique<TH1D>("h", "h", 100, 0, 300));
...
//        std::make_pair(Systematic::CMS_PileUpPtHF_j, Systematic::Down),
    ...
    CategoryTree cat_tree([](const EventDescription& desc) { return desc.is_sl && desc.numJets>=6 && desc.nBCSVM>=4; });
    auto output = make_unique<OutputNode>(
        [](const EventDescription& desc) {return desc.jets.at(0).lv.Pt();},
        [](Systematic::SystId syst_id, const EventDescription& desc) {return desc.weights.at(Systematic::syst_id_nominal);},
        [](Systematic::SystId syst_id, const EventDescription& desc) {return Systematic::is_nominal(desc.syst_id) && Systematic::is_nominal(syst_id);}
    ...
    cat_tree.outputs.push_back(std::move(output));
    cat_tree.outputs.push_back(std::move(output_systev));
    cat_tree.outputs.push_back(std::move(output_systw));
    ...
    std::string cat_name = "sl_jge6_tge4";

    outfile.cd();
    for (const auto& output : cat_tree.outputs) {
        for (auto& syst_histo : output->histograms) {
            ...
            histogram->SetName((cat_name + std::string("__") + syst_name).c_str());

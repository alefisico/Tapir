...
for sd in sets_data:
    ...
    datasets[name] = {
        ...
        "script": 'DS_heppy_crab_script_data.sh'
    }
datasets.update({
    'ttHTobb': {
...
for k in ["TTbar_inc"]: #, "QCD1000", "JetHT-Run2016B-PromptReco-v1"]:
    D = deepcopy(datasets[k])
    if k == "ttHTobb":
	D["perjob"] = 2 #for ttH target 500 ev/job => 4 LSs => 8hrs/job
    else:
	D["perjob"] = 26 #for ttbar target 8000 ev/job => 52 LSs => 6hrs/job
...
""".format(config.JobType.scriptExe, " ".join(config.JobType.scriptArgs)).strip() + '\n'
    ...
    config.JobType.inputFiles = [
        ...
        'DS_heppy_crab_script.py',
        'DS_mem_crab_script.py',
        'python.tar.gz',
        'data.tar.gz',
        "MEAnalysis_heppy.py",

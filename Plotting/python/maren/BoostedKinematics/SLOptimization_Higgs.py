...
for l in full_file_names:
    ...
    for event in ttree :
        ...
        nFat["sl"]["HCA15"].Fill(len(event.FatjetCA15SoftDrop),event.Generator_weight) #These are after basic pt, eta cuts!
        nFat["sl"]["HAK8"].Fill(len(event.FatjetAK8),event.Generator_weight) #These are after basic pt, eta cuts!
        ...
        if len(GenHiggsBosonPt) > 0:
            Count["sl"].Fill(GenHiggsBosonPt[0].pt,event.Generator_weight)
        ...
        if sample == "ttH":
            ...
            if len(CA15sorted):
                drs["sl"]["HCA15"].Fill(event.GenHiggsBoson[0].pt, Get_DeltaR_two_objects(CA15sorted[0],event.GenHiggsBoson[0]),event.Generator_weight)
            if len(AK8sorted):
                drs["sl"]["HAK8"].Fill(event.GenHiggsBoson[0].pt, Get_DeltaR_two_objects(AK8sorted[0],event.GenHiggsBoson[0]),event.Generator_weight)
            ...
            if len(GenHiggsBosonPt) > 0:
                for i in event.FatjetCA15SoftDrop:
                    if Get_DeltaR_two_objects(i,GenHiggsBosonPt[0])<1.0:
                        matchedFatjet["sl"]["HCA15"].Fill(GenHiggsBosonPt[0].pt,event.Generator_weight)
                        break

                for i in event.FatjetAK8:
                    if Get_DeltaR_two_objects(i,GenHiggsBosonPt[0])<1.0:
                        matchedFatjet["sl"]["HAK8"].Fill(GenHiggsBosonPt[0].pt,event.Generator_weight)
            ...
            for i in event.FatjetCA15SoftDrop: 
                Distributions["sl"]["HCA15"][i.matched]["pt"].Fill(i.pt,event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["eta"].Fill(i.eta,event.Generator_weight)
                btag1 = event.FatjetCA15SoftDropSubjets[i.subJetIdx1].btag
                btag2 = event.FatjetCA15SoftDropSubjets[i.subJetIdx2].btag
                Distributions["sl"]["HCA15"][i.matched]["btag1"].Fill(max(btag1,btag2),event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["btag2"].Fill(min(btag1,btag2),event.Generator_weight)
                setattr(i,"btagf",max(btag1,btag2))
                setattr(i,"btags",min(btag1,btag2))
                Distributions["sl"]["HCA15"][i.matched]["bbtag"].Fill(i.bbtag,event.Generator_weight)
                MatchedUngroomedFatjet = MatchToCand(i,event.FatjetCA15)
                Distributions["sl"]["HCA15"][i.matched]["mass"].Fill(MatchedUngroomedFatjet.mass,event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["softdropmass"].Fill(i.mass,event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["tau21"].Fill(i.tau21,event.Generator_weight)
            ...
            #And fill some basic distributions  
            for i in event.FatjetAK8: 
                Distributions["sl"]["HAK8"][i.matched]["pt"].Fill(i.pt,event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["eta"].Fill(i.eta,event.Generator_weight)
                btag1 = event.FatjetAK8Subjets[i.subJetIdx1].btag
                btag2 = event.FatjetAK8Subjets[i.subJetIdx2].btag
                Distributions["sl"]["HAK8"][i.matched]["btag1"].Fill(max(btag1,btag2),event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["btag2"].Fill(min(btag1,btag2),event.Generator_weight)
                setattr(i,"btagf",max(btag1,btag2))
                setattr(i,"btags",min(btag1,btag2))
                Distributions["sl"]["HAK8"][i.matched]["bbtag"].Fill(i.bbtag,event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["mass"].Fill(i.mass,event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["softdropmass"].Fill(i.msoftdrop,event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["tau21"].Fill(i.tau21,event.Generator_weight)
            ...
            for i in event.FatjetCA15SoftDrop:
                ...
                if MatchedUngroomedFatjet is not None:
                    ...
                    Distributions["sl"]["HCA15"][i.matched]["mass"].Fill(-100,event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["softdropmass"].Fill(i.mass,event.Generator_weight)
                Distributions["sl"]["HCA15"][i.matched]["tau21"].Fill(i.tau21,event.Generator_weight)
            ...
            for i in event.FatjetAK8: 
                i.matched = "unmatched"
                Distributions["sl"]["HAK8"][i.matched]["pt"].Fill(i.pt,event.Generator_weight)
                Distributions["sl"]["HAK8"][i.matched]["eta"].Fill(i.pt,event.Generator_weight)
                if i.subJetIdx1 != -1 and i.subJetIdx2 != -1:
                    btag1 = event.FatjetAK8Subjets[i.subJetIdx1].btag
                    btag2 = event.FatjetAK8Subjets[i.subJetIdx2].btag
                    Distributions["sl"]["HAK8"][i.matched]["btag1"].Fill(max(btag1,btag2),event.Generator_weight)
...
results = ROOT.TFile("SLOptimization_Higgs.root","recreate")
for i in cats:
    Count[i].Write()
    for j in scenarios:
        nFat[i][j].Write()

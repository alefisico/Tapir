...
brs = ["mass", "nsub", "bbtag", "btagf", "btags", "fromhiggs"]
...
for i_chunk in range(params["n_chunks"]):
    ...
    n_samples = len(root_numpy.root2array(infname_train, treename="t2", branches=["fromhiggs"], start=i_start, stop=i_start+step).view(np.recarray))

...
def train_scikit(clf):
    ...
    X = clf.image_fun(df)
    y = df["fromhiggs_new"].values
            
    tmp_weight_dic = df["fromhiggs_new"].value_counts().to_dict()    
    weight_dic = {}
...
def rocplot(clf, 
    ...
    print "data",df.loc[df["fromhiggs"] == sig_class,prob]
    ...
    # Signal 
    h1 = make_df_hist((nbins*200,min_prob,max_prob), df.loc[df["fromhiggs"] == sig_class,prob])

    # Background
    h2 = make_df_hist((nbins*200,min_prob,max_prob), df.loc[df["fromhiggs"] == bkg_class,prob])    
...
def plot_distributions(data1, data2, column=None, grid=True,
    ...
    for i, col in enumerate(com._try_sort(data1.columns)):
        ...
        df = data1[col].dropna().values
        plt.plot(df);
        #ax.hist(data1[col].dropna().values,
        #        bins=bins, range=(low,high), label = "Higgs", **kwds)
...
def analyze(clf,param_grid,esig,ebkg):
    ...
    bg = df_test.fromhiggs < 0.5
    sig = df_test.fromhiggs > 0.5
    correlations(df_test[bg].drop(['fromhiggs',"fromhiggs_new"], 1),"bkg")
    correlations(df_test[sig].drop(['fromhiggs',"fromhiggs_new"], 1),"sig")        
    ...
    plot_distributions(df_test[df_test.fromhiggs<0.5], df_test[df_test.fromhiggs>0.5],
                      column=["mass",
                              "nsub",
                              #"nsub2",
                              #"ptdr",
    ...
    y_test = df_test["fromhiggs_new"].values
    y_train = df_train["fromhiggs_new"].values
    ...
    tmp_weight_dic = df_train["fromhiggs_new"].value_counts().to_dict()    
    weight_dic = {}
    for k,v in tmp_weight_dic.iteritems():
        weight_dic[k] = float(len(X_train))/v 

    # and apply to both samples
    weights_train = np.vectorize(weight_dic.get)(df_train["fromhiggs_new"].values)
    weights_test  = np.vectorize(weight_dic.get)(df_test["fromhiggs_new"].values)

    # Generate category matrix
    matrix = np.zeros((len(clf.classes),len(clf.classes)))
    for true_class in clf.classes:        
        num = float( len(df_test.loc[ (df_test["fromhiggs_new"] == true_class),"fromhiggs"]))
        for found_class in clf.classes:            
            matrix[true_class][found_class] = len(df_test.loc[ (df_test["fromhiggs_new"] == true_class) & (df_test["output"] == found_class),"fromhiggs"])/num
    ...
    for true_class in clf.classes:
        ...
        min_prob = min([min( df_test.loc[ df_test["fromhiggs_new"] == true_class, "proba_{0}".format(reco_class) ]) for reco_class in clf.classes])
        max_prob = max([max( df_test.loc[ df_test["fromhiggs_new"] == true_class, "proba_{0}".format(reco_class) ]) for reco_class in clf.classes])
        ...
        for reco_class in clf.classes:
            ...
            # Test Sample
            prob = df_test.loc[ df_test["fromhiggs_new"] == true_class, "proba_{0}".format(reco_class) ]            
            plt.hist(prob, 
            ...
            # Train Sample
            prob = df_train.loc[ df_train["fromhiggs_new"] == true_class, "proba_{0}".format(reco_class) ]            
            plt.hist(prob, 

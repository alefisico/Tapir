...
int main(int argc, char** argv) {
    ...
    */
    ...
    ConfigurationReader configuration("../ttbb_FullyLeptonic/ttbb_TTToFullyLeptonic.lua");
    ...
    while (myReader.Next()) {
        ...
        if ( ( (*numLep) > 1 ) and ( bjets.size() > 3 ) ){
          ...
          // Compute the weights!
          std::vector<std::pair<double, double>> weights = weight.computeWeights({lep_minus, bjet1, lep_plus, bjet2, bjet3, bjet4}, met_p4);  /// order here has to match order in particles ids.
    ...
    out_tree->SaveAs("ttbb_TTToFullyLeptonic_weights.root");

...
##### How to run: python plotTF.py -o Plots/ -f RhalphabetResults/fitDiagnostics.root
###########################################
...
if __name__ == '__main__':
    ...
    plotTFsmooth(*TF_smooth_plot(*TF_params(_values, nrho=rhodeg, npt=ptdeg), minPt, maxPt, minMass, maxMass), MC=False,
                 out=args.output_folder+'/TF_'+('MC' if args.isMC else 'Data')+'_ptDeg'+str(ptdeg)+'rhoDeg'+str(rhodeg)+'_'+args.year, year=args.year,
                 label=('MC' if args.isMC else 'Data')+' TF({},{})'.format(rhodeg, ptdeg))

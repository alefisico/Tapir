...
from utils import lvec, autolog

class LeptonAnalyzer(Module):
    """
    ...
    **** this description is outdated ***
    ...
    def analyze(self, event):
        ...
        event.mu = list(Collection(event, 'Muon'))
        LOG_MODULE_NAME.debug("nanoAOD muons: " + str([(x.pt, x.eta, x.phi, x.tightId) for x in event.mu]))

        event.el = list(Collection(event, 'Electron'))
        LOG_MODULE_NAME.debug("nanoAOD electrons: " + str([(x.pt, x.eta, x.phi, x.cutBased) for x in event.el]))

...
from utils import MET, SystematicObject, lvec
import logging
...
class JetAnalyzer(Module):
    ...
    def variateJets(self, jets, systematic, sigma):
        """Recalculate pt and mass of Jet depending on jet systamtic
    ...
    def analyze(self, event):
        ...
        event.MET = MET(metobj=met)
        event.MET_gen = MET(pt=genMET.pt, phi=genMET.phi)
        ...
        #Loop over all systematic variations to finde change in category, number of jets or tags
        for syst in evdict.keys():
            ...
            if not pass_nominal or nj != nj_nominal or nt != nt_nominal: #DS
                ...
                if not event.catChange: #DS
                    LOG_MODULE_NAME.debug(syst+" invokes catChange from (j,b): {0},{1} to {2},{3}".format(nj_nominal,nt_nominal,nj,nt))
                    if not pass_nominal:
                        LOG_MODULE_NAME.debug("   --> nominal did not pass")
                    ########################## check this
                    #event.catChange = deepcopy( evdict[syst] ) #requires change in BufferedTree class
                    event.catChange = evdict[syst]  #requires change in BufferedTree class
    ...
    def _process(self, event, evdict):
        ...
        injets = list(Collection(event, 'Jet'))
        #pt-descending input jets
        #if LOG_MODULE_NAME.parent.level <= 10:
        for ij, j in enumerate(injets):
            LOG_MODULE_NAME.debug("InJetReco {0},pt = {1:06.2f}, eta = {2:05.2f},  deepcsv = {3:1.2f}, jetID = {4}, PUID = {5} ".format(ij, j.pt, j.eta, j.btagDeepB, j.jetId, j.puId))
        ...
        if LOG_MODULE_NAME.parent.level <= 10:
            ...
            for ix,x in enumerate(event.loose_jets):
                LOG_MODULE_NAME.debug("Jet: {0} - pt = {1:06.2f}, eta= {2:05.2f}, btag= {3:01.4f}".format(ix, x.pt, x.eta, x.btagDeepB))
        ...
        if "debug" in self.conf.general["verbosity"]:
            autolog("All jets: ", len(injets))
            for ix, x in enumerate(injets):

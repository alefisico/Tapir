...
class BTagLRAnalyzer(Module):
    ...
    def __init__(self, cfg_ana, isMC, sample):
        ...
        for algo in ["btagCSVV2", "btagDeepB"]:
    ...
    def analyze(self, event):
        if event.catChange: #DS
            if "systematics" in self.conf.general["verbosity"]:
                LOG_MODULE_NAME.debug("BLRAna: processing catChange")
...
#DS FIXME - allow TTbar MC to have CRs...                        or "TT" in self.sample
                elif ( not self.isMC or "TT" in self.sample or "QCD" in self.sample ) and (event.btag_LR_4b_2b > self.conf.mem["FH_bLR_4b_CR_lo"] and event.btag_LR_4b_2b < self.conf.mem["FH_bLR_4b_CR_hi"]):
                    event.selected_btagged_jets = event.btagged_jets_maxLikelihood_4b
                    event.buntagged_jets = event.buntagged_jets_maxLikelihood_4b
                    event.fh_region = 2
                    #if event.systematic == "nominal":
                    #    print "BTagLRAna: considered 4b CR event" #DS temp
                elif (not self.isMC or "TT" in self.sample or "QCD" in self.sample ) and (event.btag_LR_3b_2b > self.conf.mem["FH_bLR_3b_CR_lo"] and event.btag_LR_3b_2b < self.conf.mem["FH_bLR_3b_CR_hi"]):
                    event.selected_btagged_jets = event.btagged_jets_maxLikelihood_3b
                    event.buntagged_jets = event.buntagged_jets_maxLikelihood_3b
                    event.fh_region = 3
                    #if event.systematic == "nominal":
            ...
            if self.conf.jets["untaggedSelection"] != "btagLR" and event.is_fh and (not "ttH" in self.sample): #DS

...
def el_baseline_tight(el):
    ...
    #sca = abs(el.etaSc)
    sca = abs(el.deltaEtaSC + el.eta )

    ret = ( el.cutBased >= 4 and #tight ID as per nanoAOD bitmap mapper
            not ( sca >= 1.4442 and
                  sca < 1.5669 )
            and (
                (el.dz < 0.10 and sca <= 1.479) #Barrel
...
class Conf:
    leptons = {
        "mu": {
            ...
            "isotype": "pfRelIso04_all", #pfRelIso - delta-beta, relIso - rho
        },
    ...
    jets = {
        ...
        "btagWPs": {
            "CSVL": ("btagCSVV2", 0.5803),
            "CSVM": ("btagCSVV2", 0.8838),
            "CSVT": ("btagCSVV2", 0.9693),

            #Note: these working points are currently NOT correct
            "DeepCSVL": ("btagDeepB", 0.1522),
            "DeepCSVM": ("btagDeepB", 0.4941),
            "DeepCSVT": ("btagDeepB", 0.8001),

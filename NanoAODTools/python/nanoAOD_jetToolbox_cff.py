...
def setupCustomizedJetToolbox(process, isMC):
...
def nanoJTB_customizeMC(process):
    run2_nanoAOD_94X2016.toModify(process, setupCustomizedJetToolbox(process, True))
    run2_nanoAOD_94XMiniAODv2.toModify(process, setupCustomizedJetToolbox(process, True))
    run2_nanoAOD_102Xv1.toModify(process, setupCustomizedJetToolbox(process, True))
    process.NANOAODSIMoutput.fakeNameForCrab = cms.untracked.bool(True)  # needed for crab publication
    return process

def nanoJTB_customizeDATA(process):
    run2_nanoAOD_94X2016.toModify(process, setupCustomizedJetToolbox(process, False))
    run2_nanoAOD_94XMiniAODv2.toModify(process, setupCustomizedJetToolbox(process, False))
    run2_nanoAOD_102Xv1.toModify(process, setupCustomizedJetToolbox(process, False))
